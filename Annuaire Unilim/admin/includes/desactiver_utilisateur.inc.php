...
if(isset($_SESSION['ID_ADMIN'],$_GET['description'], $_GET['id_modifier'])){
    ...
    if($description == 'etudiant'){
        $activer_query = "UPDATE `annuaireunilim`.`etudiant` SET `COMPTE_ACTIVE` = '0' WHERE (`ID_ETUDIANT` = $id);";
    }else{
        $activer_query = "UPDATE `annuaireunilim`.`fonctionnaire` SET `COMPTE_ACTIVE` = '0' WHERE (`ID_FONCTIONNAIRE` = $id);";
    }
